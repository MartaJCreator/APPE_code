...
class Bell_Burnell(object):

    """
    Named aftery Dame Jocyln Bell Burnell for data science work in
    discoverying pulsar starts
    Bell_Burnell collects the output from the other objects to compile and
    ...
    def __init__(self):
        ...
        """
        When Bell_Burnell is created, they create a version of Helper to analyse
        data
        """
        print("Bell_Burnell here")
        self.Helper = Helper()
        pass

    def collect_files(self, Brooks):

        self.list_o_files = Brooks.get_list_o_files()
        self.location_o_files = Brooks.get_location_o_files()
        pass
...
class Helper(object):
    ...
    def finding_reference (self):
        ...
        if "_capillary_" in self.file_name:
            self.type_of_dataset = "capillary"
    ...
    def process_peaks (self):
        ...
        if self.type_of_dataset == 'capillary':
...
#        pass
    ...
    def peak_possitions_ref (self):
        ...
        Currently set for finding peaks in a capillary data set as the
...
#                print(line)
    ...
    def find_resolution_ref (self):
        ...
        works best for the capillary data (high peak intensity values)
        """
#        conversion_factor = self.data_array[0,1] - self.data_array[0,0]
...
class CentralDataHolder (object):
    ...
    def ouput_align_peaks (self):
        ...
        for DataHolder in self.collection_o_datasets:
            ...
            if DataHolder.type_of_data == "capillary":

                """
                Change to REF
                """

                capillary_DataHolder = DataHolder
        ...
        for DataHolder in list_o_SX_DataHolder:
            ...
            DataHolder.peak_match(capillary_DataHolder.peaklist_40)
...
#    intput_variables = input("please enter important information")
    ...
    """
    a_Brooks.read_files() is where the folder of data is given, by changing
    ...
    a_Brooks = Brooks()
    a_Brooks.read_files("test-data")

    # a_Brooks.read_files("background-removed_sep20data")

    a_Bell_Burnell = Bell_Burnell()

    a_Bell_Burnell.collect_files(a_Brooks)

    a_Bell_Burnell.opener_o_files(a_CentralDataHolder)
